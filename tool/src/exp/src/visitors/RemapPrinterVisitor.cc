...
RemapPrinterVisitor::RemapPrinterVisitor(
    const std::unordered_map<std::string, std::string> &targetToRemap,
    Language lang, bool colored, PrintMode mode)
    : ExpVisitor(), _ss(), _lang(lang), _colored(colored),
      _printMode(mode), _ope_stack(), _temporal_ope_stack(),
      _targetToRemap(targetToRemap) {
  _ope_stack.push(ope::ope::NoOp);
  _temporal_ope_stack.push(ope::temporalOpe::TemporalNoOp);
}

void RemapPrinterVisitor::clear() {
  _ss.clear();
  _ss.str(std::string());
}

std::string RemapPrinterVisitor::get() {
...
#define VARIABLE(LEAF)                                               \
  void RemapPrinterVisitor::visit(LEAF &o) {                         \
    if (_targetToRemap.count(o.getName())) {                         \
      _ss << selCol(_targetToRemap.at(o.getName()),                  \
                    VAR(_targetToRemap.at(o.getName())));            \
    } else {                                                         \
...
#define EXP_OPE(NODE)                                                \
  void RemapPrinterVisitor::visit(expression::NODE &o) {             \
    if (dynamic_cast<Proposition *>(&o) != nullptr) {                \
      std::string tmp_str =                                          \
          prop2String(*dynamic_cast<Proposition *>(&o));             \
      if (_targetToRemap.count(tmp_str)) {                           \
        _ss << selCol(_targetToRemap.at(tmp_str),                    \
                      VAR(_targetToRemap.at(tmp_str)));              \
        return;                                                      \
      }                                                              \
    }                                                                \
...
#define UNARY_FUNCTION(TYPE)                                         \
  void RemapPrinterVisitor::visit(TYPE &o) {                         \
    if (_colored) {                                                  \
...
bool RemapPrinterVisitor::sereNeedsCurlyBrackets() {
...
static bool isBinaryRightAssociative(ope::temporalOpe op) {
...
void RemapPrinterVisitor::visit(
...
void RemapPrinterVisitor::visit(SereFirstMatch &o) {
  bool needsCurlyBrackets = sereNeedsCurlyBrackets();
  auto [open, close] = getSereBrackets();
...
void RemapPrinterVisitor::visit(SereConcat &o) {
  bool needsCurlyBrackets = sereNeedsCurlyBrackets();
  auto parent_op = _temporal_ope_stack.top();
  bool putBrackets =
...
void RemapPrinterVisitor::visit(SerePlus &o) {
  bool needsCurlyBrackets = sereNeedsCurlyBrackets();
  auto [open, close] = getSereBrackets();
