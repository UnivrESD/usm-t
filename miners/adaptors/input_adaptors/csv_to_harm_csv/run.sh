...
if [ ! -d "$out_path" ]; then
    echo "Error: Output path '$out_path' is not a directory."
    exit 1
fi
...
if [ -f "$in_path" ]; then
    ...
    if [[ "$in_path" == *.csv ]]; then
        ...
        echo "Error: Input file '$in_path' does not have a .csv extension."
        exit 1
    fi
# Check if input is a directory
    ...
    find "$in_path" -maxdepth 1 -type f -name "*.csv" -exec cp {} "$new_out_dir" \;
else
    echo "Error: Input path '$in_path' is not a valid file or directory."
    exit 1
fi
